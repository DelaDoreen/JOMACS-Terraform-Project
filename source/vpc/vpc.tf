...
resource "aws_route_table" "project_private_rt" {
  ...
  tags = {
    Name = var.project_private_rt_name
  }
}


resource "aws_route_table_association" "project_rta_pub" {
  subnet_id      = aws_subnet.public_subnet.id
  route_table_id = aws_route_table.project_pub_rt.id
}


resource "aws_route_table_association" "project_rta_pr" {
